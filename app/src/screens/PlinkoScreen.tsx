...
const MULTIPLIERS_LOW = [8.9, 3, 1.4, 1.1, 1, 0.5, 1, 1.1, 1.4, 3, 8.9];
const MULTIPLIERS_MED = [18, 4, 1.7, 1.3, 1, 0.5, 0.3, 0.5, 1, 1.3, 1.7, 4, 18];
const MULTIPLIERS_HIGH = [110, 41, 10, 5, 3, 1.5, 1, 0.5, 0.3, 0.5, 1, 1.5, 3, 5, 10, 41, 110];
...
export default function PlinkoScreen({ onBack }: { onBack: () => void }) {
  ...
  const dropBall = () => {
    ...
    if (!bet || bet < 1 || bet > balance || dropping) return;
    ...
    Animated.sequence(anims).start(() => {
      ...
      setTimeout(() => resultScale.setValue(0), 2000);
    });
  };
  ...
  return (
    <View style={s.screen}>
      ...
      <View style={s.boardWrap}>
        <View style={[s.board, { width: BOARD_W, height: BOARD_W * 0.85 }]}>
          {/* Pegs */}
      ...
      {lastResult && (
        <Animated.View style={[s.resultBadge, { transform: [{ scale: resultScale }] }]}>
          ...
          <Text style={s.resultPayout}>+{lastResult.payout}</Text>
        </Animated.View>
      )}
